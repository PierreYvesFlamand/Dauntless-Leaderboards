...
CREATE TABLE `trial_leaderboard_items_players` (`trial_leaderboard_item_id` INT NOT NULL , `trial_leaderboard_item_type_id` INT NOT NULL , `player_id` INT NOT NULL , `platform_id` INT NOT NULL , `objectives_completed` INT NOT NULL , `completion_time` INT NOT NULL , `weapon_id` INT NOT NULL , `role_id` VARCHAR(50) NOT NULL , `rank` INT NOT NULL ) ENGINE = InnoDB;
ALTER TABLE `trial_leaderboard_items_players` ADD FOREIGN KEY (`trial_leaderboard_item_id`) REFERENCES `trial_leaderboard_items`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `trial_leaderboard_items_players` ADD FOREIGN KEY (`trial_leaderboard_item_type_id`) REFERENCES `trial_leaderboard_item_type`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `trial_leaderboard_items_players` ADD FOREIGN KEY (`player_id`) REFERENCES `players`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `trial_leaderboard_items_players` ADD FOREIGN KEY (`platform_id`) REFERENCES `platforms`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `trial_leaderboard_items_players` ADD UNIQUE(`trial_leaderboard_item_id`, `trial_leaderboard_item_type_id`, `player_id`, `rank`);