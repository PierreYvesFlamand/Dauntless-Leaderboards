...
SELECT g.id AS guild_id, gd.icon_filename as guild_icon_filename, g.name AS guild_name, g.tag AS guild_tag, glig.`level`, glig.remaining_sec
FROM gauntlet_leaderboard_items gli
...
LEFT JOIN guild_data gd ON gd.guild_id = g.id
WHERE gli.gauntlet_season = 12 AND gli.last_updated = (
...
-- Get guild seasons ranks
SELECT gs.season, glig.`rank`
FROM gauntlet_seasons gs
LEFT JOIN gauntlet_leaderboard_items gli ON gli.gauntlet_season = gs.season
LEFT JOIN gauntlet_leaderboard_items_guilds glig ON glig.gauntlet_leaderboard_item_id  = gli.id 
lEFT JOIN guilds g ON g.id = glig.guild_id 
WHERE g.id = 1 AND gli.last_updated = (
	SELECT MAX(last_updated)
	FROM gauntlet_leaderboard_items gli
	WHERE gli.gauntlet_season = gs.season
)

-- Get guild details
SELECT g.id, gd.icon_filename, gd.discord_link, gd.detail_html, g.name, g.tag, (100 / 646) * SUM((100 - glig.rank) * (gs.season / 12)) as rating, SUM(glig.rank = 1) as nbrTop1, SUM(glig.rank <= 5) as nbrTop5, SUM(glig.rank <= 20) as nbrTop20, SUM(glig.rank <= 100) as nbrTop100, SUM(glig.level) as totalLevelCleared
FROM gauntlet_seasons gs
LEFT JOIN gauntlet_leaderboard_items gli ON gli.gauntlet_season = gs.season
LEFT JOIN gauntlet_leaderboard_items_guilds glig ON glig.gauntlet_leaderboard_item_id  = gli.id 
lEFT JOIN guilds g ON g.id = glig.guild_id 
lEFT JOIN guilds_data gd ON gd.guild_id = glig.guild_id 
WHERE g.id = 5 AND gli.last_updated = (
	SELECT MAX(last_updated)
	FROM gauntlet_leaderboard_items gli
	WHERE gli.gauntlet_season = gs.season
)
...
-- Get guild list
SELECT g.id, gd.icon_filename, g.name, g.tag, (100 / 646) * SUM((100 - glig.rank) * (gs.season / 12)) as rating, SUM(glig.rank = 1) as nbrTop1, SUM(glig.rank <= 5) as nbrTop5, SUM(glig.rank <= 20) as nbrTop20, SUM(glig.rank <= 100) as nbrTop100, SUM(glig.level) as totalLevelCleared
FROM gauntlet_seasons gs
LEFT JOIN gauntlet_leaderboard_items gli ON gli.gauntlet_season = gs.season
LEFT JOIN gauntlet_leaderboard_items_guilds glig ON glig.gauntlet_leaderboard_item_id  = gli.id 
lEFT JOIN guilds g ON g.id = glig.guild_id 
lEFT JOIN guilds_data gd ON gd.guild_id = glig.guild_id 
WHERE (g.name LIKE '%%' || g.tag LIKE '%%') AND gli.last_updated = (
	SELECT MAX(last_updated)
	FROM gauntlet_leaderboard_items gli
	WHERE gli.gauntlet_season = gs.season
)
GROUP BY g.id
ORDER BY rating DESC, g.name ASC, g.tag ASC
